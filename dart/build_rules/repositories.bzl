...
# Reuse the build file content from extensions.bzl
load(":extensions.bzl", "dart_repositories_extension", "_DART_SDK_BUILD_FILE")

def dart_repositories(sdk_channel="",sdk_version="", linux_x64_sha="", macos_arm64_sha="", macos_x64_sha="", use_default=False):
    """Legacy WORKSPACE function for Dart repositories.
    
    This function is maintained for backward compatibility with pre-Bazel 8.0.
    For Bazel 8.0+, use MODULE.bazel instead.
    """
    if use_default:
        _sdk_channel = "stable"
        _sdk_version = "2.17.7"
